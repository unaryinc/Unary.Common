...
int main(int argc, const char** argv) {
	...
	for(std::string line; std::getline(in, line);) {
		...
		if(!std::isxdigit(line[tabcount]) || tabcount >= 3) //-V781
			continue;
	...
	std::sort(std::end(vendor_id_indices), std::end(vendor_id_indices), [](auto&& a, auto&& b) { return a.pci_id < b.pci_id; }); //-V539
